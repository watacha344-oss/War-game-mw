...
export const BALANCE = {
  ...
  units: {
    inf: {
      name: '歩兵大隊', en: 'Infantry Bn', men: 780, veh: 14, vehLabel: 'Veh', speed: 5, roadBonus: 1.3, terrain: [1, 0.7, 0.8, 0, 1],
      vision: 260, range: 130, attack: 1.6, ammoUse: 0.5, fuelUse: 0, foodUse: 0.12,
      maxFuel: 0, maxAmmo: 100, maxFood: 100, noAmmo: 0.15,
    },
    tank: {
      name: '戦車大隊', en: 'Armor Bn', men: 320, veh: 46, vehLabel: 'AFV', speed: 8, roadBonus: 1.7, terrain: [1, 0.4, 0.6, 0, 1],
      vision: 300, range: 170, attack: 2.6, ammoUse: 0.7, fuelUse: 0.3, foodUse: 0.12,
      maxFuel: 100, maxAmmo: 100, maxFood: 100, noAmmo: 0,
    },
    arty: {
      name: '砲兵大隊', en: 'Artillery Bn', men: 360, veh: 24, vehLabel: 'Guns', speed: 5, roadBonus: 1.6, terrain: [1, 0.35, 0.5, 0, 1],
    ...
    supply: {
      name: '補給大隊', en: 'Supply Bn', men: 210, veh: 64, vehLabel: 'Trucks', speed: 7, roadBonus: 1.8, terrain: [1, 0.3, 0.45, 0, 1],
      vision: 200, range: 90, attack: 0.4, ammoUse: 0.2, fuelUse: 0.12, foodUse: 0.12,
      maxFuel: 100, maxAmmo: 100, maxFood: 100, noAmmo: 0,
    },
    recon: {
      name: '偵察大隊', en: 'Recon Bn', men: 190, veh: 28, vehLabel: 'AFV', speed: 10, roadBonus: 1.8, terrain: [1, 0.55, 0.7, 0, 1],
      vision: 480, range: 110, attack: 0.9, ammoUse: 0.3, fuelUse: 0.12, foodUse: 0.12,
      maxFuel: 100, maxAmmo: 100, maxFood: 100, noAmmo: 0,
    },
    para: {
      name: '空挺大隊', en: 'Airborne Bn', men: 700, veh: 8, vehLabel: 'Veh', speed: 5, roadBonus: 1.3, terrain: [1, 0.75, 0.85, 0, 1],
...
// ================= 5) 画面（UI） =================
// 作戦指示画面（C4ISR風）の設定。時刻・ターン表示・航空優勢・フェーズライン・編成ツリーなど。

const _line = (f) => Array.from({ length: 13 }, (_, i) => [Math.round(f(i * 150)), i * 150]);

export const UI = {
  title: 'OPERATIONAL COMMAND: DIVISION LEVEL',
  divisionName: '1st Infantry Division (Mechanized)',
  clockStartMinutes: 6 * 60,   // 作戦開始時刻（0600）
  turnSeconds: 60,             // 1ターン＝実時間60秒
  turnClockMinutes: 10,        // 1ターンで作戦時刻が進む分数（TURN 12 → 0800 HRS）
  historyInterval: 10,         // 損害・補給グラフの記録間隔（秒）
  historyMax: 200,             // グラフに残す点の最大数
  intelMemory: 90,             // 発見した敵の「最後の確認位置」を地図に残す秒数
  // 航空優勢（青の割合%）= base + perUseLeft×航空支援の残り回数 + perKillDiff×(敵の損失−味方の損失)
  air: { base: 55, perUseLeft: 6, perKillDiff: 2.5, min: 15, max: 85 },
  phaseLines: [
    { name: 'PHASE LINE BLUE', color: '#4a90e2', points: _line((y) => riverX(y) - 130) },
    { name: 'PHASE LINE RED', color: '#e25a4a', points: _line((y) => 2150 + 40 * Math.sin(y / 250)) },
  ],
  villages: [
    { x: 600, y: 730, n: 7 }, { x: 1040, y: 520, n: 5 },
    { x: 2010, y: 650, n: 8 }, { x: 1930, y: 1240, n: 6 },
  ],
  groups: [
    { label: 'Infantry Battalions', types: ['inf'] },
    { label: 'Armor Battalions', types: ['tank'] },
    { label: 'Artillery Battalions', types: ['arty'] },
    { label: 'Recon / Airborne', types: ['recon', 'para'] },
    { label: 'Supply Battalions', types: ['supply'] },
  ],
};
